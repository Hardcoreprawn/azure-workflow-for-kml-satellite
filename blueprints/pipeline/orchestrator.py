...
def treesight_orchestrator(context: df.DurableOrchestrationContext):  # type: ignore[return-type]
    ...
    # Extract user_id early for quota refund on failure
    ...
    try:
        ...
        context.set_custom_status(
            {"phase": "ingestion", "step": "storing_claims", "aois": len(aois)}
        )
        aoi_refs = cast(
            "list[dict[str, str]]",
            (
        ...
        context.set_custom_status(
            {"phase": "acquisition", "step": "searching", "aois": len(aoi_refs)}
        )
        ...
        for i in range(0, len(aoi_refs), acq_batch_size):
            ...
            acq_tasks = [
                context.call_activity(activity, _acq_payload(ref, inp, composite))
        ...
        # Poll orders
        context.set_custom_status(
            {"phase": "acquisition", "step": "polling", "orders": len(orders)}
        )
        poll_tasks = [
        ...
        # --- Phase 3: Fulfilment ---
        context.set_custom_status(
            {"phase": "fulfilment", "step": "downloading", "ready": len(ready)}
        )
        output_container = inp.get("output_container", DEFAULT_OUTPUT_CONTAINER)
        ...
        if batch_ready:
            ...
            while pending:
                ...
                pending = [
                    t for t in batch_tracking if t.get("state") not in ("completed", "failed")
                ]
                if pending:
        ...
        pp_batch_size = config_get_int(
            inp, "post_process_batch_size", DEFAULT_POST_PROCESS_BATCH_SIZE
        )
        ...
        # Refund the quota slot consumed at submission time
