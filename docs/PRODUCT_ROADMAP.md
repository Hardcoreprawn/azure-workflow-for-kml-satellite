...
| 5.8 | **ESA CCI Fire / Burned Area integration** (global 250 m monthly burned-area grids, 2001–present) | Medium | 5.4 | Complements FIRMS hotspot data (5.4) with validated burned-extent polygons and detection dates. GeoTIFF download from ESA ODP → cache in blob → clip-to-AOI during enrichment. Free & open. |
...
| 6.8 | **ESA CCI Land Cover integration** (annual 300 m land-cover maps, 1992–2020, 22 LCCS classes) | High | 3.8 | Adds 28-year historical land-cover baseline — detect transitions (forest → cropland → urban). Bulk GeoTIFF from ESA ODP → blob cache → clip-to-AOI. Pairs with 6.4 (Landsat baselines) for long-term trend context. Free & open. |
| 6.9 | **ESA CCI Biomass integration** (global above-ground biomass, 100 m, 2007–2022) | High | 6.8 | Quantifies carbon / biomass loss in Mg/ha — essential for ESG (EUDR, TNFD) and carbon-credit MRV. Multi-epoch differencing enables "this AOI lost X Mg C between 2017–2022". GeoTIFF from ESA ODP → blob cache. Free & open. |
...
| **ESA CCI data pipeline** (bulk GeoTIFF download → blob cache → clip-to-AOI) | M5 (5.8), M6 (6.8, 6.9) | Medium | Shared integration pattern for all three ESA CCI products; build once, reuse |
