...
| #1289 | fix(dev): extend dev image staleness guard to detect `Dockerfile.dev`/`pyproject.toml`/`rust/` recipe drift, not just `uv.lock` — new `org.canopex.build-inputs-sha256` OCI label + `--print-build-digest` CLI mode; review found and fixed a real bug where the digest swept in Cargo's gitignored `rust/target/` build output, defeating determinism (closes #1115). |
