...
| #1219 | feat(ci): wire `make test-pipeline-local` into GitHub Actions as a required `pipeline-e2e` job — runs the real pipeline (parse → trigger → orchestrate → acquire → fulfil → enrich → artifact) against Azurite on every PR; bare-runner design documented as an ADR 0005 amendment (closes #1218). |
| #1217 | feat(e2e): local/CI pipeline e2e gate — `CANOPEX_TEST_MODE` stub-provider gating + `make test-pipeline-local`, proving the full pipeline runs end-to-end against Azurite with no live Azure environment or third-party network calls (closes #1216). |
