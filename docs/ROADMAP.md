...
| #832 | fix(upload): auto-create personal org on first submission for unaffiliated users — replaces hard 403 (regression from #831) with self-healing personal org; re-verifies association via `get_user_org()` after create to catch silent failures and concurrent races. |
| #831 | fix(account): PR review — owner authz on invite list, token replay protection, Cosmos SQL ARRAY_CONTAINS syntax, CIAM config split, base64url decode, userId format, XSS DOM API, discoverApiBase on init — 8 new tests (27 total in test_org.py). ✅ |
| #830 | feat(account): org management + invite lifecycle + GDPR endpoints — JWT invite token signing/validation (HS256, 7-day TTL), `PATCH /api/user/profile`, `DELETE /api/user` with ownership transfer, org/member/invite endpoints, account frontend (profile, org, delete modal), public invite accept page, E2E tests, ACCOUNT_GDPR_GUIDE.md, ACCOUNT_USER_GUIDE.md. 1779 tests passing. |
