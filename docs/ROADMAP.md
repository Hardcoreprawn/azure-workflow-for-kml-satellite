...
| PR | Summary |
|----|---------|
| — | feat(account/slices4–7): frontend scaffold + profile/delete/org/invite pages — #830 **Slice 4** (`1ebebf7`): `/account/` directory with responsive single-page layout, auth guard overlay, sidebar nav (Profile/Organization/Settings), hash-based routing. Vanilla JS, no build step, integrates with canopex-auth.js. **Slice 5** (`65a3b49`): Profile form PATCH /api/user/profile (display_name validation), delete modal DELETE /api/user (transfer_to user ID), form loading via GET /api/user, real-time error handling (400/403/404/503). **Slice 6** (`9e59784`): Organization members list with owner/member display, member removal (owner-only), pending invites with revoke (owner-only), invite form (owner-only). **Slice 7** (`9cd4b6f`): Public invite acceptance page `/account/invite/?token={jwt}` — no auth required on load, JWT token validation/expiration check, invite details display (org, email), Sign In → Accept flow for unauthenticated users, POST /api/org/invites/{token}/accept on acceptance, success redirect to /account/#organization. All 1766 tests passing, zero regressions. Next: Slice 8 (E2E tests + GDPR docs). |
