...
| #1397 | refactor(deploy): consolidate `deploy-infra` job outputs to come directly from the single `Capture Tofu outputs` step, and remove a duplicated orchestrator-only rollback capture/restore path in favour of the existing canonical rollback step (closes #745). |
