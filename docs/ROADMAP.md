...
| PR | Summary |
|----|---------|
| — | feat(account/slices1–8): complete account management + GDPR compliance — #830 ✅ **ALL SLICES COMPLETE** **Slices 1–2** (JWT lifecycle): `generate_invite_token()` HS256 signing with 7-day TTL, `validate_invite_token()` signature + expiration verification (8 tests, `87b1890`). **Slices 3** (HTTP endpoints): `PATCH /api/user/profile` + `DELETE /api/user` with ownership transfer logic (13 tests, `5b91c82`). **Slices 4–7** (Frontend): `/account/` scaffold (auth guard, sidebar nav, responsive layout), profile form + delete modal, org members + invites, public invite accept page at `/account/invite/?token={jwt}` with token validation & expiration checks (1766 baseline tests, `1ebebf7`+`65a3b49`+`9e59784`+`9cd4b6f`). **Slice 8** (E2E + GDPR docs): 13 workflow tests (user creation, org setup, invite acceptance, profile update, account deletion with transfer, cascading deletion, GDPR rights), ACCOUNT_GDPR_GUIDE.md (full GDPR Article 15–20 compliance: access, rectification, erasure, portability, restrictions), ACCOUNT_USER_GUIDE.md (end-user account management workflows). **Total: 1779 tests passing (+13 E2E), zero regressions.** Backend: 8 HTTP endpoints. Frontend: 4 pages. Docs: 2 compliance guides. Next: Stage 2D revenue enablement. |
| — | feat(account/slice3): PATCH /api/user/profile & DELETE /api/user endpoints — #830 **Slice 3** (`5b91c82`) — HTTP API endpoints for user account management: `update_profile_endpoint` for display name updates with request/field validation; `delete_account_endpoint` for GDPR-compliant cascading deletion with ownership transfer logic. Both endpoints use `@require_auth` decorator, return proper HTTP status codes (200/204/400/403/404/503), and have full error handling. Service layer (`update_user_profile`, `delete_user`) integrated. Registered account blueprint. 13 new endpoint tests covering auth, validation, error cases, success paths. Total 84 tests passing, zero regressions. |
| #823 | feat(billing): stages 2–4 — org-pooled run accounting implementation — **Stage 2** (wire orchestrator): orchestrator calls `finalize_run(status)` activities on pipeline completion/failure, guarded for paid+demo tiers; **Stage 3** (deprecate quota): POST /api/analysis/submit wired to `reserve_run()` with full error handling; dead quota functions removed from upload/orchestrator/activities; **Stage 4** (frontend display): new `GET /api/billing/pool-status` endpoint returns org pool snapshot (allowance, reserved, completed, refunded, available, per_member). Replaces all old quota/billing code. All 1733 tests passing; 4 new endpoint tests. ✅ Completes umbrella #814 (Stage 1 from #821 + Stages 2–4 here). |
