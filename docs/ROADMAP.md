...
| PR | Summary |
|----|---------|
| — | feat(account/slices4–6): frontend scaffold + profile/delete/org members UI — #830 **Slice 4** (`1ebebf7`): `/account/` directory with responsive single-page layout, auth guard overlay, sidebar nav (Profile/Organization/Settings), hash-based routing. Vanilla JS, no build step, integrates with canopex-auth.js. **Slice 5** (`65a3b49`): Profile form PATCH /api/user/profile (display_name validation), delete modal DELETE /api/user (transfer_to user ID), form loading via GET /api/user, real-time error handling (400/403/404/503). Backend endpoints added: GET /api/user (profile fetch), GET /api/org/invites (list pending), PATCH /api/org/invites/{email}/revoke, POST /api/org/invites/{token}/accept. **Slice 6** (`9e59784`): Organization members list with owner/member display, member removal (owner-only), pending invites with revoke (owner-only), invite form (owner-only) with email validation, real-time API calls for member management. Integrates with GET /api/org/members, DELETE /api/org/members/{id}, GET/PATCH /api/org/invites endpoints. All 1766 tests passing (84 user/org, 1682 cross-module), zero regressions. Next: Slice 7 (invite acceptance page) & Slice 8 (E2E tests). |
