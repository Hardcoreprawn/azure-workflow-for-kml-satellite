...
| PR | Summary |
|----|---------|
| — | feat(account/slices4–5): frontend scaffold + profile/delete UI — #830 **Slice 4** (`1ebebf7`): `/account/` directory with responsive single-page layout, auth guard overlay, sidebar nav (Profile/Organization/Settings), hash-based routing. Vanilla JS, no build step, integrates with canopex-auth.js. **Slice 5** (`65a3b49`): Profile form PATCH /api/user/profile (display_name validation), delete modal DELETE /api/user (transfer_to user ID), form loading via GET /api/user, real-time error handling (400/403/404/503). Backend endpoints added: GET /api/user (profile fetch), GET /api/org/invites (list pending), PATCH /api/org/invites/{email}/revoke, POST /api/org/invites/{token}/accept. All 1766 tests passing, zero regressions. Next: Slice 6 (org members UI) & Slice 7 (invite acceptance page). |
