...
| #1305 | refactor(frontend): split `website/js/app-evidence-display.js` (1071 lines) into `app-evidence-display-{map,selection,ai}.js` sibling modules, preserving the `window.CanopexEvidenceDisplay` public API; fixed a reset-button handler-stacking bug found during the extraction (closes #1291). |
| #1304 | test(harness): coverage hardening phase 2 — brings `treesight/pipeline/eudr.py` (the core EUDR determination logic), `ai/client.py`, `storage/client.py`, `payment_provider.py`, `_blob_url.py`, and `diagnostics.py` to ≥80% coverage via behavior-level failure/fallback-path tests (closes #1290). |
| #1296 | feat(compat): instrument three legacy/compat shims (per-user quota preservation, `_resolve_legacy_user_org`, AOI metrics `year_a`/`year_b` dual-key support) with `LEGACY_COMPAT_HIT` warning logs + a Legacy/Compat Support Matrix in `docs/OPERATIONS_RUNBOOK.md`; review found the PR's removal-tracking issue links were fabricated (pointed at unrelated pre-existing issues) and that no tests covered the new logging — filed real tracking issues (#1298, #1299, #1300) and added 11 regression tests asserting each signal fires only on its exact precondition (closes #888). |
| #1294 | test(harness): coverage hardening phase 1 — 94 new unit tests bring `blueprints/pipeline/activities.py`, `blueprints/pipeline/aoi_orchestrator.py`, and `treesight/providers/planetary_computer.py` from ~36–38% to ≥80% coverage (planetary_computer reaches 100%); review found and fixed two real CodeQL-flagged bugs (a generator-driving test pattern that could leave a variable referenced-before-assignment) plus an unused-variable notice (closes #886). |
| #1293 | refactor(pipeline): split `blueprints/export.py` (1683 lines) into `blueprints/export/{geojson,csv,pdf,audit_pdf}.py` submodules — pure extraction verified function-by-function to be byte-identical to the original, only import paths changed in tests (closes #887). |
